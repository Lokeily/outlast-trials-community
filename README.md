...
下方曲线是**真实数据**：每个独立访客当天首次打开网站任意页面，计数后端就 +1（同一人当天重复访问不重复计数），并按北京时间归并到当天。曲线由 GitHub Actions 每天 00:00（北京时间）自动从计数后端拉取、重新生成，再同步进本 README——不再依赖任何估算。

![每日浏览人数曲线](assets/visitors-chart.svg)

- **计数后端**：Cloudflare Worker + KV，按独立访客去重计当日人数，存储为「日期 → 当天人数」。
...
- 左下角「当前在线访客」（实时并发人数，每 30 秒刷新）与首页「今日访问人次」（当日去重累计）同为计数后端的**实时真实数据**；与本图「每日累计浏览人数」口径一致，区别仅在本图按天归并、每天 00:00 才更新一次。
