...
下方曲线是**真实数据**：每次有人打开网站任意页面，计数后端就 +1，并按北京时间归并到当天。曲线由 GitHub Actions 每天 00:00（北京时间）自动从计数后端拉取、重新生成，再同步进本 README——不再依赖任何估算。

![每日浏览人数曲线](assets/visitors-chart.svg)

- **计数后端**：Cloudflare Worker + KV，每次访问 +1，按「日期 → 当天人数」存储。
- **图表同步**：`.github/workflows/update-visitor-chart.yml` 定时运行，调用 `/stats` 接口生成 `assets/visitors-chart.svg` 并提交。
- **防刷**：仅统计来自本站、且非爬虫 / 预览的真实访问。
- 周末（周五~周日）柱子用更亮的红色，便于一眼看出周末来看攻略的人更多。
- 左下角的「当前在线访客」数字是另一套独立的实时估算（基于作息曲线），与本图的「每日累计浏览人数」口径不同。
